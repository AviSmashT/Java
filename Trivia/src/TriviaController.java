...
public class TriviaController {
    ...
    final private int POINT = 10; // points for correct answer
    final private int PENALTY = -5; // Wrong answer penalty
    ...
    void OnSubmt(ActionEvent event) {
    	if (toggledBtn() == NOT_SELECTED) { // if no answer was selected by player (-1)
			...
			game.setScore(game.getScore() + PENALTY); // wrong answer penalty
    ...
    private boolean gameEnded(ActionEvent event) {
		int userInput = JOptionPane.showConfirmDialog(null, "Your score is: " + game.getScore() + "/" + (game.getlevels() * POINT) + 
				"\nDo you want to keep playing? (Restart game)", "Message" ,JOptionPane.YES_NO_OPTION, JOptionPane.QUESTION_MESSAGE);
